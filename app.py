...
class HFEndpointAPI:
    def __init__(
        self, model_name, api_url, hf_token, answer_tokens, repetition_penalty
    ):
        self.tokenizer = AutoTokenizer.from_pretrained(model_name, token=hf_token)
        # Inference Endpoints for large models
        if model_name == "bigscience/bloom-7b1":
            self.api_url = (
                "https://w3xj0fk1w4zqcb3m.us-east-1.aws.endpoints.huggingface.cloud"
            )
        else:
...
def main(model, max_new_tokens=1, repetition_penalty=1.0):
    ...
    model_api = HFEndpointAPI(
        model, ENDPOINT_API, SHADES_TOKEN, max_new_tokens, repetition_penalty
    )
